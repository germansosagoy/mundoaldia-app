...
export function RandomArticle({ data }: { data: newsType }) {
  ...
  return (
    ...
    <div>
    <div href={data?.url} className='relative w-[300px] h-[200px]'>
      <Image src={`${data?.urlToImage ? data?.urlToImage : "/image/main-news.svg"}`} alt={data?.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 100vw, 100vw" className='object-cover rounded-sm' />
    </div>
      <Link href={data?.url} legacyBehavior>
        <a target="_blank" className='text-sm hover:underline'>{data?.title}</a>
      </Link>
